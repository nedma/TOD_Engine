...
public class EnviroVegetationInstance : MonoBehaviour {
	...
	void Start () 
	{
		...
		{
			Age.startAgeinHours = UnityEngine.Random.Range (0f, maxAgeInHours);
	...
	void CalculateScale ()	
	{
		if (rescale)
		{
			...
			float agemod = (ageInHours / maxAgeInHours) * GrowSpeedMod;

			currentVegetationObject.transform.localScale = minScale + new Vector3(agemod,agemod,agemod);
