...
public class HTOD_Scattering : HTOD_ImageEffect
{
	...
	[HTOD_Range(0, 4)] public int BlurIterations = 2;
	...
	protected void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		if (SinglePass)
		{
			scatteringMaterial.EnableKeyword("HTOD_SCATTERING_SINGLE_PASS");
		}
		else
		{
			scatteringMaterial.DisableKeyword("HTOD_SCATTERING_SINGLE_PASS");
		}

		Shader.SetGlobalTexture("HTOD_BayerTexture", DitheringTexture);
		Shader.SetGlobalVector("HTOD_ScatterDensity", new Vector4(HeightFalloff, ZeroLevel, GlobalDensity, StartDistance));
