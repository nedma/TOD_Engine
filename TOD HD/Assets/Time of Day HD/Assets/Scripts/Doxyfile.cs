...
[ <a href="https://forum.unity.com/threads/time-of-day-dynamic-sky-dome.172763/">Discussion</a> | <a href="https://andererandre.github.io/HTOD/demo/">Demo</a> | <a href="https://andererandre.github.io/HTOD/docs/">Documentation</a> ]
...
The script HTOD_Time manages the dynamic day & night cycle.

The following parameters are being set by HTOD_Time:
- HTOD_Sky.Cycle.Hour
- HTOD_Sky.Cycle.Day
- HTOD_Sky.Cycle.Month
- HTOD_Sky.Cycle.Year
...
The following events are fired by HTOD_Time:
- HTOD_Time.OnMinute
- HTOD_Time.OnHour
- HTOD_Time.OnDay
- HTOD_Time.OnMonth
...
1. Backup the current set of parameters using the export function in the context menu of the HTOD_Sky script
2. Import the new version of Time of Day from the Asset Store
3. Check the HTOD_Sky script on your prefab to see if there are any setup issues that have to be addressed
...
5. Check the HTOD_Sky script on your prefab to see if there are any setup issues that have to be addressed
...
The HTOD_Sky.World and HTOD_Sky.Cycle parameter sections allow for configuration of the sky dome to simulate the exact earth, sun and moon movement for any location on the planet depending on Gregorian date, UTC/GMT time zone and geographic coordinates. This for example allows to recreate eclipses just as they would occur in real life.

It is important to manually set the correct time zone offset (HTOD_Sky.World.UTC) that fits the longitude and latitude parameters in order to use local time instead of UTC.
...
- Ray color is managed by HTOD_Sky
...
The HTOD_Sky script sets some global shader parameters that can be used in your custom shaders. For a complete list see the HTOD_Base.cginc file. Any of those variables can be used in any shader by simply defining uniform variables with the same name, which will then automatically be set to the most recent values every frame. It is also possible to simply include HTOD_Base.cginc to get access to all variables.

In addition to those base variables there is also HTOD_Scattering.cginc, which offers functions to easily evaluate the scattering equations in custom shaders.

The file HTOD_Clouds.cginc contains cloud-related functions to render cloud layers and cloud shadows.



\n \section networking Networking

- To network date and time, synchronize the property HTOD_Sky.Cycle.Ticks of type long
- To network cloud movement, synchronize the property HTOD_Sky.Components.Animation.CloudUV of type Vector3



\n \section serialization Parameter Import & Export

It is possible to import and export custom presets to XML via the "Import Parameters" and "Export Parameters" options in the context menu of the HTOD_Sky script. Exported parameters can be imported in other projects or even loaded at runtime by using the appropriate API calls.
...
Q: How can I get a sky dome (HTOD_Sky) reference in my custom scripts?
- HTOD_Sky.Instance keeps a static reference to the most recent sky dome that has been instantiated.
- HTOD_Sky.Instances keeps a static list of referenes to all sky domes that have been instantiated.

Q: How can I use the sky dome with virtual reality devices like the Oculus Rift?
- Add the HTOD_Camera script to one of the cameras (preferably the one that's being rendered first).
...
Q: How can I get the god rays (HTOD_Rays) to consider or ignore a certain object?
- HTOD_Rays uses the depth buffer and color buffer alpha values to block light.
...
- Disable the "Single Pass" flag on the HTOD_Scattering image effect.
...
\n \section changelog Changelog
    ...
    - Added HTOD_Sky.SaveParameters to export parameters at runtime
    - Improved HTOD_Scattering fog density calculation
    - Fixed a bug in the star position calculation
    - Fixed color space and color range parameters affecting HTOD_Scattering fog density
    - Fixed HTOD_Sky.SunriseTime and HTOD_Sky.SunsetTime not being converted to the local time zone

    VERSION 3.2.0
    -------------
    - Added support for Unity 5.5 and 5.6
    - Added support for single-pass stereo rendering
    - Added directional scattering occlusion to HTOD_Scattering
    - Added gradient fog mode that uses the unmodified gradient color
    - Added camera movement parameter to HTOD_Animation
    - Added single-pass toggle to HTOD_Scattering (disable when using AA / planar water reflections)
    ...
    - Replaced cloud shadow projector with the new HTOD_Shadows image effect
    - Added LDR support to the HTOD_Scattering image effect (no longer forces HDR)
    - Fixed HTOD_Scattering being brighter towards screen edges
    - Fixed reflection probe bake ignoring sky dome rotation
    - Improved sun spot shape calculation
    - Added HTOD_Sky.Ambient.Saturation parameter (saturate or desaturate ambient light)
    - Added HTOD_Sky.ColorOutput selection (toggles anti-banding dithering)
    - Added HTOD_Sky.Day/Night.FogColor (Atmosphere.Fogginess uses this instead of the cloud color)
    - Added HTOD_Time.ProgressTime toggle
    - Added HTOD_Time.OnSunrise and HTOD_Time.OnSunset events
    - Made HTOD_Time.DayLengthInMinutes accept values that are less than one
    - Added sky dome setup issue detection to HTOD_Sky
    - Added more tooltips to the HTOD_Sky inspector and made various tooltips more descriptive
    ...
    - Moved parameter import and export to HTOD_Sky context menu
    - Made weather manager an example script instead of a sky dome component
    - Added HTOD_ prefix to all example scripts
    ...
    VERSION 3.0.2
    -------------
    - Added UseDeviceDate to HTOD_Time (UseDeviceTime now only sets the time of day, not the date)
    ...
    - Replaced HTOD_TRANSFORM_VERT macro with direct MVP mul in cloud shadow and skybox shaders
    ...
    - Forced HTOD_Time.DayLengthInMinutes to be greater than or equal to one
    - Removed the custom HTOD_Camera inspector script
    - Adjusted render queue positions of all shaders
    - Added global density and height falloff to the HTOD_Scattering image effect
    ...
    - Added events that are fired when a year, month, day, hour or minute have passed to HTOD_Time
    - Added an image effect that renders atmospheric scattering and aerial perspective in a single pass
    - Added profiler samples to HTOD_Sky
    ...
    - Simplified and optimized HTOD_Time calculations
    ...
    - Removed Progress* fields from HTOD_Time (no longer required)
    - Removed Moon (Flat) shader (adjusting Moon.Contrast now has the same effect)

    VERSION 2.3.3
    -------------

    - Added HTOD_Sky.LoadParameters(...) to load exported parameters at runtime
    - Added LoadSkyFromFile example script
    - Added skybox material that is assigned to the render settings skybox for dynamic GI
    - Added HTOD_Sky.Moon.HaloSize to increase or decrease the size of the moon halo
    - Added HTOD_Sky.Reflection.ClearFlags to specify which clear flags to use for the reflection cubemap
    - Added HTOD_Sky.Reflection.CullingMask to specify which layers to include in the reflection cubemap
    - Added warning to HTOD_Camera if skybox clear flags are used (redundant with a sky dome)
    ...
    - Changed HTOD_Sky.RenderToSphericalHarmonics(...) and HTOD_Sky.RenderToCubemap(...) APIs
    - Renamed HTOD_Components.*Shader to HTOD_Components.*Material
    - Removed HTOD_Sky.Fog.UpdateInterval (it's fast enough to update every frame anyhow)
    - Removed HTOD_Sky.Fog/Ambient/Reflection.Directional (now part of fog mode, unused for the others)
    ...
    - Renamed HTOD_AmbientType.Flat to HTOD_AmbientType.Color
    - Renamed HTOD_AmbientType.Trilight to HTOD_AmbientType.Gradient
    - Renamed HTOD_Sky.RenderToSH3(...) to HTOD_Sky.RenderToSphericalHarmonics(...)

    VERSION 2.3.1
    -------------

    - Fixed errors if sky dome renderers or mesh filters were deleted (i.e. when running on a server)
    - Fixed that ScatteringColor(...) in HTOD_Scattering.cginc would add some stuff to its alpha value
    - Fixed issues if the main camera of a scene changes after scene load
    - Added HTOD_Sky.World.Horizon to specify whether or not to adjust the horizon to zero level
    - Added HTOD_Sky.UpdateFog(), HTOD_Sky.UpdateAmbient() and HTOD_Sky.UpdateReflection() to API
    - Added headless mode detection to skip some rendering calculations when running on a server
    - Made HTOD_Sky.SampleAtmosphere(...) only include the moon halo if directLight is true
    - Made the moon halo always fade out when the moon is below the horizon
    - Made HTOD_Sky.Cycle.DateTime have DateTimeKind.Utc instead of DateTimeKind.Unspecified
    - Made the fog color values clamp between 0 and 1 to avoid super bright glowing directional fog
    - Changed HTOD_AdditiveColor and HTOD_MoonHaloColor in HTOD_Base.cginc to float3 (alpha is unused)
    - Removed HTOD_Components.CameraTransform as it is no longer required
    ...
    - Added HTOD_Animation.RandomInitialCloudUV to randomize the clouds at startup
    - Added optional shader "Moon (Flat)" for a flatter moon shading
    - Added HTOD_Sky.World.ZeroLevel to set the zero / water level of a scene
    - Added HTOD_Camera.DomePosOffset to specify a sky dome position offset relative to the camera
    - Added HTOD_Sky.Initialized to check whether or not the sky dome has been initialized
    ...
    - Moved more enums to the global namespace and added the HTOD_ prefix
    - Moved Cycle.Longitude, Cycle.Latitude and Cycle.UTC to the World parameter category
    - Changed the returned alpha value of HTOD_Sky.SampleAtmosphere(...) to one
    - Changed the returned alpha value of ScatteringColor(...) in HTOD_Scattering.cginc to one
    - Renamed HTOD_Sky+Variables to HTOD_Sky+API (now contains all API methods and properties)
    - Renamed HTOD_Sky+Quality to HTOD_Sky+Settings (now sets all project and scene settings)
    - Renamed HTOD_SunShafts to HTOD_Rays (now handles god rays of both sun and moon)
    - Renamed HTOD_Sky.SunShaftColor to HTOD_Sky.RayColor
    - Renamed HTOD_Sky.Light.ShaftColoring to HTOD_Sky.Light.RayColoring
    - Renamed HTOD_Sky.Sun.ShaftColor to HTOD_Sky.Sun.RayColor and added HTOD_Sky.Moon.RayColor
    - Removed HTOD_Sky.World.HorizonOffset and HTOD_Sky.World.ViewerHeight (now covered by ZeroLevel)
    - Removed HTOD_AmbientType.Hemisphere since it was removed from Unity 5 (use trilight instead)
    - Removed clampAlpha parameter from HTOD_Sky.SampleAtmosphere(...)
    - Replaced HTOD_Sky.Ambient.Exposure with Day.AmbientMultiplier and Night.AmbientMultiplier
    - Replaced HTOD_Sky.Reflection.Exposure with Day.ReflectionMultiplier and Night.ReflectionMultiplier
    ...
    - Added HTOD_Sky.Stars.Position to specify whether or not to move the stars with the earth rotation
    - Added HTOD_Sky.SampleAtmosphere(...) overload that ignores direct light
    - Added HTOD_Sky.RenderToCubemap(...) with various overloads
    - Added HTOD_Sky.RenderToSH3(...) with various overloads
    - Added HTOD_Sky.SampleFogColor(), HTOD_Sky.SampleSkyColor() and HTOD_Sky.SampleEquatorColor()
    - Added optional shader to project cubemaps onto the space object
    - Removed HTOD_Sky.FogColor (access RenderSettings.fogColor instead)
    - Removed HTOD_Sky.Stars.Density (directly adjust the texture instead)
    - Moved all fog parameters to HTOD_Sky.Fog
    - Moved all ambient light parameters to HTOD_Sky.Ambient
    - Moved all reflection parameters to HTOD_Sky.Reflection
    ...
    - Fixed time not properly incrementing in some cases if HTOD_Time.ProgressDate was checked
    ...
    - Rescaled HTOD_Sky.Light.CloudColoring (custom prefabs have to be readjusted accordingly)
    - Rescaled HTOD_Sky.Night.CloudMultiplier (custom prefabs have to be readjusted accordingly)
    - Added HTOD_Sky.Day.CloudColor and HTOD_Sky.Night.CloudColor
    - Added HTOD_Sky.Instance and HTOD_Sky.Instances to easily get the most recent sky or all skies in the scene
    - Added HTOD_Animation.WorldSpaceCloudUV
    - Added overloads of T() and ScatteringColor() that take distance into account to HTOD_Scattering.cginc
    - Removed HTOD_Base.cginc include from HTOD_Scattering.cginc (now has to be included in the shader file)
    ...
    - Added HTOD_Scattering.cginc that contains functions to sample the scattering color
    - Added HTOD_Base.cginc that contains shader parameters and common transformations
    - Added HTOD_World2Sky and HTOD_Sky2World shader matrices
    - Added HTOD_Sky.Stars.Brightness parameter to make stars get affected by bloom image effects
    - Added HTOD_Sky.LocalMoonDirection, HTOD_Sky.LocalSunDirection and HTOD_Sky.LocalLightDirection
    - Added HTOD_Sky.Sun.MeshBrightness and HTOD_Sky.Moon.MeshBrightness
    - Added HTOD_Sky.Sun.MeshContrast and HTOD_Sky.Moon.MeshContrast
    - Added HTOD_Sky.Clouds.Glow to adjust the light source glow applied to the clouds
    - Added HTOD_Sky.Atmosphere.FakeHDR to adjust the fake HDR mapping that is applied at dusk and dawn
    - Added HTOD_Time.TimeCurve to specify a time progression curve for the day night cycle
    - Added two new cloud textures (the old ones can be deleted if unused)
    - Removed two unnecessary calls to InverseTransformDirection from HTOD_Sky.SampleAtmosphere
    ...
    - Made HTOD_Sky.Cycle.DateTime accurate to one millisecond rather than one second
    - Made camera scripts automatically search for the sky dome if no reference is set in the inspector
    - Moved all moon parameters to HTOD_Sky.Moon.X (was HTOD_Sky.Night.MoonX and HTOD_Sky.Cycle.MoonX)
    - Moved all sun parameters to HTOD_Sky.Sun.X (was HTOD_Sky.Day.SunX)
    ...
    - Tweaked the HTOD_Sky.IsDay and HTOD_Sky.IsNight thresholds
    - Replaced HTOD_Time.UpdateInterval with HTOD_Sky.Light.UpdateInterval (now only affects the light source)
    ...
    - Added inspector variable to adjust the time update interval in HTOD_Time
    - Added option to use the real-life moon position rather than the fake "opposite to sun" moon position
    - Made all components of HTOD_Sky initialize before Start() so that they are accessible from other scripts
    ...
    - Fixed HTOD_Camera always causing the scene to be edited if enabled
    ...
    - Added a property for the atmosphere renderer component to HTOD_Components
    - Added properties for all child mesh filter components to HTOD_Components
