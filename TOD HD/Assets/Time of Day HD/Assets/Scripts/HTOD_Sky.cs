...
[ExecuteInEditMode]
[RequireComponent(typeof(HTOD_Resources))]
[RequireComponent(typeof(HTOD_Components))]
public partial class HTOD_Sky : MonoBehaviour
{
	...
	private void UpdateScattering()
	{
        ...
        //[devinwei] whatever it is, this value shall be a constant
        //it represents the irradiance of sunlight reaching top of atmosphere long way from sun.
        //however, due to different game engine, the value may vary because of different energy conserving policies
	...
	private void UpdateCelestials()
	{
		...
		float lst_rad, sun_zenith_rad, sun_altitude_rad, sun_azimuth_rad, moon_zenith_rad, moon_altitude_rad, moon_azimuth_rad;
		{
			...
			// Moon position
			if (Moon.Position == HTOD_MoonPositionType.Realistic)
			{
				// See http://www.stjarnhimlen.se/comp/ppcomp.html#4
		...
		// Transform updates
		{
			...
			if (Stars.Position == HTOD_StarsPositionType.Rotating)
			{
				#if UNITY_EDITOR
		...
		// Color calculations
		{
			...
			SunLightColor  = HTOD_Util.ApplyAlpha(Day.LightColor.Evaluate(dayTime));
			MoonLightColor = HTOD_Util.ApplyAlpha(Night.LightColor.Evaluate(nightTime));

			// Sun and moon light ray colors
			SunRayColor  = HTOD_Util.ApplyAlpha(Day.RayColor.Evaluate(dayTime));
			MoonRayColor = HTOD_Util.ApplyAlpha(Night.RayColor.Evaluate(nightTime));

			// Sky color
			SunSkyColor  = HTOD_Util.ApplyAlpha(Day.SkyColor.Evaluate(dayTime));
			MoonSkyColor = HTOD_Util.ApplyAlpha(Night.SkyColor.Evaluate(nightTime));

			// Mesh color
			SunMeshColor  = HTOD_Util.ApplyAlpha(Day.SunColor.Evaluate(dayTime));
			MoonMeshColor = HTOD_Util.ApplyAlpha(Night.MoonColor.Evaluate(nightTime));

			// Cloud color
			SunCloudColor  = HTOD_Util.ApplyAlpha(Day.CloudColor.Evaluate(dayTime));
			MoonCloudColor = HTOD_Util.ApplyAlpha(Night.CloudColor.Evaluate(nightTime));

			// Fog color
			Color dayFogColor   = HTOD_Util.ApplyAlpha(Day.FogColor.Evaluate(dayTime));
			Color nightFogColor = HTOD_Util.ApplyAlpha(Night.FogColor.Evaluate(nightTime));
			FogColor = Color.Lerp(nightFogColor, dayFogColor, LerpValue);

			// Ambient color
			Color dayAmbientColor   = HTOD_Util.ApplyAlpha(Day.AmbientColor.Evaluate(dayTime));
			Color nightAmbientColor = HTOD_Util.ApplyAlpha(Night.AmbientColor.Evaluate(nightTime));
			...
			// Moon halo color
			MoonHaloColor = HTOD_Util.MulRGB(MoonSkyColor, Moon.HaloBrightness * moonAboveHorizon);
			...
			#endif
			{
                Components.LightSource.intensity = Mathf.LinearToGammaSpace(lightIntensity);
			}
