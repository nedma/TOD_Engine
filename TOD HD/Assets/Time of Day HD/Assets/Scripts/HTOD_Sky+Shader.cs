...
{
	private const int HTOD_SAMPLES = 2;
	...
	private Color HTOD_HDR2LDR(Color color)
	{
	...
	private Color HTOD_GAMMA2LINEAR(Color color)
	{
		return new Color(color.r * color.r, color.g * color.g, color.b * color.b, color.a);
	}

	private Color HTOD_LINEAR2GAMMA(Color color)
	{
	...
	private Color ShaderScatteringColor(Vector3 dir, bool directLight = true)
	{
		...
		float sampleLength  = far / (float)(HTOD_SAMPLES);
		...
		for (int i = 0; i < (int)(HTOD_SAMPLES); i++)
		{
